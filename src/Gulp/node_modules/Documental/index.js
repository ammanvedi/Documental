	...
	Utils.determineFunctions = function ( sourceString, acornInstance, walk ) {
		...
		walk( ast, function ( node ) {
			...
			if ( node.type == 'FunctionDeclaration' ) {
				...
				/*
				body = sourceString.substring( node.start, node.end );
				par = Utils.paramsToStringRepresentation( Utils.getSimpleParameters( node ) );
				methodCount++;
				doc = Utils.findComments( node.loc );
				if ( doc == -1 ) {
					newdocstring = Utils.autoGenerateDocStub( sourceString, node, node.id.name );
					doc = doctrine.parse( newdocstring, { unwrap: true } );
				}
				Utils.addMemberToSourceMap( sourceMap, node.id.name, doc, body );
			...
			if ( node.type == 'VariableDeclaration' ) {
				node.declarations.forEach( function ( declaration, declIndex ) {
					if ( declaration.init && declaration.init.type == 'FunctionExpression' ) {
						...
						/*
						body = sourceString.substring( node.start, declaration.init.end );
						par = Utils.paramsToStringRepresentation( Utils.getSimpleParameters( declaration.init ) );
						methodCount++;
						doc = Utils.findComments( declaration.init.loc );
						if ( doc == -1 ) {
							newdocstring = Utils.autoGenerateDocStub( sourceString, declaration.init, name );
							doc = doctrine.parse( newdocstring, { unwrap: true } );
						}
			...
			if ( node.type == 'ExpressionStatement' ) {
				...
				if ( node.expression && node.expression.left && node.expression.right && node.expression.left.type == 'MemberExpression'
					...
					if ( rightside.type == "FunctionExpression" ) {
						...
						/*
						body = sourceString.substring( node.expression.left.start, rightside.end );
	...
	Utils.recurseMenuTree = function ( sourceObject, tree, nodename, menu, workingpath ) {
		...
		if ( typeof sourceObject === "object" && sourceObject !== null && !sourceObject[ "description" ] ) {
			...
			for ( var x = 0; x < keys.length; x++ ) {
				//console.log(keys, keys[x], nodename);
				//console.log(keys[x] !== "Documentation", "\n\n\n");
